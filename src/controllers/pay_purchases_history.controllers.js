  ...
  try {
    ...
    const response = await pool.query(
      `select 
        amount,
        remaining,
        to_char( date, 'DD/MM/YYYY') as payment_date 
        from pay_purchases_history where id_invoice=$1`,
      [id]
    );
    res.status(200).json(response.rows);
  } catch (err) {
...
module.exports = {
  getPayPurchasesHistory,
  getPayPurchasesHistoryById,
  createPayPurchasesHistory,
};
